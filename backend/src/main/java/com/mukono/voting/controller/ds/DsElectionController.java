...
/**
 * REST controller for election management. Provides endpoints to create,
 * update, list, and cancel elections.
 */
@RestController
...
public class DsElectionController {
	...
	public ResponseEntity<ElectionResponse> create(@Valid @RequestBody CreateElectionRequest request) {
		var election = electionService.create(request.getName(), request.getDescription(), request.getFellowshipId(),
				PositionScope.valueOf(request.getScope()), request.getDioceseId(), request.getArchdeaconryId(),
				request.getChurchId(), request.getTermStartDate(), request.getTermEndDate(),
				request.getNominationStartAt(), request.getNominationEndAt(), request.getVotingStartAt(),
	 ...
	 */
	@PutMapping("/{id}")
	public ResponseEntity<ElectionResponse> update(@PathVariable Long id,
	 ...
	 */
	@GetMapping
	public ResponseEntity<Page<ElectionSummary>> list(@RequestParam(required = false) Long fellowshipId,
			@RequestParam(required = false) String scope, @RequestParam(required = false) String status,
			@RequestParam(required = false) Long dioceseId, @RequestParam(required = false) Long archdeaconryId,
			@RequestParam(required = false) Long churchId, @RequestParam(name = "page", defaultValue = "0") int page,
		...
		var result = electionService
				.list(fellowshipId, scopeEnum, statusEnum, dioceseId, archdeaconryId, churchId, pageable)
	...
	public ResponseEntity<ElectionResponse> cancel(@PathVariable Long id,
